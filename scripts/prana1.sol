...
contract prana is ERC721 {
    ...
    constructor() public ERC721("PranaBooks", "PBT") {
        owner = msg.sender;
    ...
    // uint16 public nonce;  // might need more work

    // struct to store book details. For each new title.
    // bytes16(encryptedBookDataHash) - the actual content of the book
    // bytes16(unencryptedBookDetailsCID) - cid to pull book cover and other details to show the world
    ...
    // uint16(bookPrice) - price of the book that the creator asks for direct purchase
    // uint16(transactionCut) - cut of further transactions on copies
    ...
    struct BookInfo {
        ...
        uint16 bookPrice;
        uint16 transactionCut;
        uint16 bookSales;
    }

    // mapping for all books
    // ISBN is the key, its corresponding details is the value
    mapping(uint16 => BookInfo) internal booksInfo;

    // struct for token details and transactions
    // uint16(isbn) binds the token to the book it points to
    // uint16(copyNumber) to count which copy of the book the token is
    // so that people can brag about owning the 1st copy, 100th copy etc,
    // add sell them at a premium
    // uint16(resalePrice) is the price that tokenOwner asks to sell the token
    ...
    // mapping (address => uint16) internal accountBalance;
    ...
    event TokenForSale(
        uint16 indexed resalePrice,
        uint16 indexed isbn,
        uint256 indexed tokenId
    );

    //Event to emit when the tokenOwner puts out a token for renting
    event TokenForRenting(
        uint16 indexed rentingPrice,
        uint16 indexed isbn,
        uint256 indexed tokenId
    );
    ...
    // function fetchMyNFTs() public view returns (uint16[] memory) {
    ...
    //         }
    //     }
    //     uint16[] memory items = new uint16[](itemCount);
    //     for (uint16 i = 0; i < totalItemCount; i++) {
    //         if (ownerOf(i) == msg.sender) {
    ...
    function setPranaHelperAddress(address _pranaHelperAddress)
        public
        onlyOwner
    {
    ...
    function _updateAccountBalances(uint256 tokenId) internal {
        ...
        (booksInfo[tokenData[tokenId].isbn].publisherAddress).transfer(
            booksInfo[tokenData[tokenId].isbn].transactionCut *
                (msg.value / 100)
        );
    ...
    function publishBook(
        string memory _encryptedBookDataHash, //TOCHECK: bytes16 vs bytes memory
        uint16 _isbn,
        uint16 _price,
        string memory _unencryptedBookDetailsCID,
        uint16 _transactionCut
    ) public {
        require(
            booksInfo[_isbn].publisherAddress == address(0),
            "This book has already been published!"
        );
        require(
            _transactionCut > 0 && _transactionCut < 80,
            "Your cut can't be more than 80% of the total"
        );
    ...
    function directPurchase(uint16 _isbn) public payable returns (bool) {
        ...
        require(
            msg.value >= booksInfo[_isbn].bookPrice,
            "Insufficient funds ! Please pay the price as set by the author."
        );
    ...
    function putTokenForSale(uint16 salePrice, uint256 tokenId) public {
        require(
    ...
    function buyToken(uint256 tokenId, address _tokenRecipient) public payable {
        ...
        require(
            msg.value >= tokenData[tokenId].resalePrice,
    ...
    function putForRent(uint16 _newPrice, uint256 tokenId) public {
        require(
            msg.sender == ownerOf(tokenId),
            "You are not this token's owner"
        );
    ...
    function rentToken(uint256 tokenId) public payable {
        ...
        require(
            msg.value >= tokenData[tokenId].rentingPrice,
            "Your price isn't sufficient to rent this copy"
        );
        require(
            msg.sender != ownerOf(tokenId),
            "Token Owner can't rent one's own token"
        );
    ...
    function setTokenURI(uint256 tokenId, string memory _uniqueCIDfortoken)
        public
    {
    ...
    function consumeContent(uint256 tokenId)
        public
        view
    ...
    // function viewBalance() public view returns(uint16){
    //     return accountBalance[msg.sender];
    // }

    //function to get book details with the tokenId
    //returns CID of coverpic+bookname
    function viewTokenDetails(uint16 _tokenId)
        public
        view
        returns (
            uint16,
        ...
        )
    {
        ...
        return (
            tokenData[_tokenId].isbn,
            booksInfo[tokenData[_tokenId].isbn].unencryptedBookDetailsCID,
            tokenData[_tokenId].copyNumber,
            tokenData[_tokenId].resalePrice,
            tokenData[_tokenId].isUpForResale
        );
    }

    //split up from viewTokenDetails, specifically for renting. To avoid Stack Too Deep Error
    function viewRentingTokenDetails(uint16 _tokenId)
        public
        view
        returns (
            uint16,
    ...
    function tokenForResaleAtIndex(uint16 index) public view returns (uint256) {
    ...
    function tokenForRentingAtIndex(uint16 index)
        public
        view
        returns (uint256)
    {
    ...
    function numberOfRentedTokens(address _rentee)
        public
        view
        returns (uint256)
    {
    ...
    function tokenOfRenteeByIndex(address _rentee, uint16 index)
        public
        view
        returns (uint256)
    {
    ...
    function viewBookDetails(uint16 _isbn)
        public
        view
        returns (
            string memory,
        ...
        )
    {
        require(
            booksInfo[_isbn].publisherAddress != address(0),
            "This book is not on the platform"
        );
        return (
            booksInfo[_isbn].unencryptedBookDetailsCID,
            booksInfo[_isbn].publisherAddress,
            booksInfo[_isbn].bookPrice,
            booksInfo[_isbn].transactionCut,
            booksInfo[_isbn].bookSales
        );
    }

    function viewMyBookDetails(uint16 _isbn)
        public
        view
        returns (
            string memory,
        ...
        )
    {
        ...
        require(
            msg.sender == booksInfo[_isbn].publisherAddress,
            "You are not this book's author"
        );
        return (
            booksInfo[_isbn].encryptedBookDataHash,
            booksInfo[_isbn].unencryptedBookDetailsCID,
            booksInfo[_isbn].publisherAddress,
            booksInfo[_isbn].bookPrice,
            booksInfo[_isbn].transactionCut,
